defmodule LcovEx do
  ...
  def start(compile_path, opts) do
    ...
    fn ->
      ...
      lcov =
        :cover.modules()
        |> Enum.sort()
        |> Enum.map(&calculate_module_coverage(&1, ignored_paths, ignore_modules, caller_cwd))
  ...
  defp calculate_module_coverage(mod, ignored_paths, ignore_modules, cwd) do
    ...
    if Path.type(path) != :relative or Enum.any?(ignored_paths, &String.starts_with?(path, &1)) or ignored?(mod, ignore_modules) do
      []
    else
